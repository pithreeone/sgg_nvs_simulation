...
cannot read as `on the chair` is not a fair question to ask a robot.
    ...
    python build_robotic_task/build_on.py --n 12 --out datasets/robot/cases_on.json
"""
...
from build_robotic_task.build_tabletop import (MIN_PIXELS, TARGET_CLASSES,
...
def class_rank(egtr, frame, want: str) -> int:
    ...
    """
    from robot.grounding import class_mass
...
def one_case(controller, rng: random.Random, index: int,
    ...
    return {
        ...
        # NO TABLE.  `robot.proc_scene.rebuild` spawns whatever `objects` names

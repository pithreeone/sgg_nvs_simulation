...
def _cast_fp16() -> None:
    """Build SEVA's three modules in fp16, by swapping the classes before use.

    Stock keeps CLIP ViT-H and the SD-2.1 VAE in fp32 and the UNet in bf16 --
    6.8 GB of weights, which OOMs at LOAD time on an 8 GB card, so casting after
    construction is too late.  fp16 drops it to ~3.5 GB.  `third_party/` is left
    untouched; this is `sgg_nvs/scratch/seva_fp16_probe.py`'s patch, applied in
    process rather than by re-running `simple_inference.py`.

    SD-2.1's VAE is documented to produce NaNs in fp16, which would show up as
    black frames.  Measured on the llff-room trajectory at T=11: 20 frames, mean
    117 and std 57 across all of them, so it does not bite here.  Check any new
    run for it anyway -- a black sweep still scores.
    """
    import seva.model as model_mod
    import seva.modules.autoencoder as ae_mod
    import seva.modules.conditioner as cond_mod

    def half_on_init(cls):
        class FP16(cls):
            def __init__(self, *args, **kwargs):
                super().__init__(*args, **kwargs)
                self.half()

        FP16.__name__ = cls.__name__ + "FP16"
        return FP16

    if not getattr(ae_mod.AutoEncoder, "_seva_fp16", False):
        ae_mod.AutoEncoder = half_on_init(ae_mod.AutoEncoder)
        ae_mod.AutoEncoder._seva_fp16 = True
        cond_mod.CLIPConditioner = half_on_init(cond_mod.CLIPConditioner)
        cond_mod.CLIPConditioner._seva_fp16 = True

        stock = model_mod.SGMWrapper

        class SGMWrapperFP16(stock):
            _seva_fp16 = True

            def __init__(self, module):
                super().__init__(module.half())

        model_mod.SGMWrapper = SGMWrapperFP16
...
class Synthesiser:
    ...
    def __init__(self, steps: int = 10, cfg: float = 3.0,
                 ...
                 dump: Optional[str] = None, device: str = "cuda:0",
        ...
        self.dump, self.device, self.fp16 = dump, device, fp16
...
def calibrate(argv_args) -> int:
    ...
    from robot.world.proc_scene import ROOM, Robot, look_from, open_room, rebuild
    ...
    model = Synthesiser(steps=argv_args.synth_steps,
                        ...
                        dump=argv_args.dump, T=argv_args.synth_T,
                        fp16=argv_args.synth_fp16)
    rows = []
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    ap.add_argument("--synth-T", type=int, default=21)

...
def visible_pixels(event, name: str) -> int:
    ...
    `build.sgg.occlusion.visible_pixels`, so occlusion and grading carry over.
...
# ---------------------------------------------------------------------------
# STOCK THOR SCENES.  `open_room` above builds a procedural room; these two open
# one of THOR's own FloorPlans instead, with the robot standing in it.  They were
# `drive.py`, a 157-line module whose other 64 lines were an interactive keyboard
# loop -- seven callers imported it for these two functions and never for that.
# ---------------------------------------------------------------------------

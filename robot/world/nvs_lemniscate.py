...
from build.sgg.geometry import bbox_of
from robot.task.task_find import build_tasks, grade, put_in_front
...
def target_mask(event, name: str) -> Tuple[int, Optional[List[int]]]:
    ...
    READS THE SEGMENTATION FRAME, not `third_party_instance_masks`.  That dict
    omits every SpawnAsset object -- it lists walls and floor and nothing else --
    which is the same defect `proc_scene.visible_pixels` documents for the agent
    camera, and it made this function return 0 for every procedural case.  The
    colour map is complete where the mask dict is not.
    """
    colour = next((c for c, oid in event.color_to_object_id.items()
                   if oid == name), None)
    frames = getattr(event, "third_party_instance_segmentation_frames", None)
    if colour is None or not frames:
        return 0, None
    frame = frames[0]
    mask = (frame == np.array(colour, dtype=frame.dtype)).all(-1)
    return (int(mask.sum()), bbox_of(mask)) if mask.any() else (0, None)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    rc = proc_scene.open_scene(args.scene, args.width, args.height, args.fov,
                          args.start)
    try:
        print(f"{args.scene}   reference {proc_scene.pose_line(rc)}")
        ...
        # message -- the trap `build/sgg/occlusion_pipeline.py` documents.  With the

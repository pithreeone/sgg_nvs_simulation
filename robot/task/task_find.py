...
def place_on_floor(rc, clone: str, near: float = 2.0) -> Optional[str]:
    ...
    from robot.task.measure import agent_masks
...
def place_beside(rc, target_name: str, distractor_name: str,
    ...
    """
    from build.sgg.occlusion import by_name, set_pose, visible_pixels
...
def stage_at(rc, target_name: str, occluder_name: str,
    ...
    """
    from build.sgg.occlusion import by_name, set_pose, set_poses, visible_pixels
...
def rc_snapshot(rc):
    ...
    from build.sgg.occlusion import pose_snapshot
...
def put_in_front(rc, target_name: str, occluder_type: str,
    ...
    `build.sgg.occlusion.occlude` does this by bisection but places the occluder at
    ...
    `build/sgg/find_walkaround.py` makes about placement in general.
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.resume:
        from robot.task.measure import resume
    ...
    try:
        print(f"{scene}   {proc_scene.pose_line(rc)}")
        if args.distractor:
